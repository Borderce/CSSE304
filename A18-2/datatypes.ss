...
(define-datatype continuation continuation?
	...
	[set!-k2
		(cell cell?)
		(k continuation?)]
	[closure-k (bodies (list-of expression?))
		(env environment?)
		(k continuation?)]
	[map-k (cps-proc scheme-value?)
		(cdrls (list-of expression?))
		(k continuation?)]
	[map-k-prim (cps-proc scheme-value?)
		(cdrls (list-of scheme-value?))
		(k continuation?)]
	[map-k2 (car-res scheme-value?)
		(k continuation?)]
	[add-global-k (id always?)
		(k continuation?)]
	[exit-k ]
	)
	...
	(lambda (k val)
		(cases continuation k
			...
			[closure-k (bodies env k)
				(closure val bodies env)]
			[map-k (cps-proc cdrls k)
				(map-cps cps-proc cdrls (map-k2 val k))]
			[map-k-prim (cps-proc cdrls k)
				(map-cps-prim cps-proc cdrls (map-k2 val k))]
			[map-k2 (car-res k)
				(apply-k k (cons car-res val))]
			[add-global-k (id k)
				(apply-k k (add-global id val))]
			[exit-k ()
				val]
			)))
