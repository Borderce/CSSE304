...
(define all-sym? (lambda (lst)
	(cond [(null? lst) #t]
		[(list? (car lst)) (if (equal? 'ref (1st (1st lst))) 'ref #f)]
	...
	(cond
     ...
     [(pair? datum)
      (cond
	   ...
	   [(eqv? 'define (1st datum)) (define-exp-p datum)]
...
(define define-exp-p (lambda (datum)
	(list 'define-exp (2nd datum) (parse-exp (3rd datum)) )))
...
(define lambda-exp-p (lambda (datum)
	(cond 
		...
		[(equal? 'ref (all-sym? (2nd datum)))
		 (list 'lambda-exp-ref (2nd datum) (parse-exp-list (cddr datum)))]
