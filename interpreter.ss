...
(define eval-exp
  (lambda (exp env k)
    (cases expression exp
	  ...
	  ;[define-exp (id body) (add-global id (eval-exp (syntax-expand body) env))]
...
(define eval-rands
  (lambda (rands env k)
    (apply-k k (map (lambda (x) (eval-exp x env (id-k (lambda (v) v)))) rands))))
...
(define *prim-proc-names* '(+));'(cons + cons - > list display))
